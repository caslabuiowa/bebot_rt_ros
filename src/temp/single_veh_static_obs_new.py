    ...
    try:
        ...
        while np.linalg.norm(vehicle.state[:2].squeeze() - goal) > 0.1:
            ...
            if t - t_last > dt:
                ...
                # If the trajectory generation was successful, save it. By setting t_last in this 'if' statement we force
                # the trajectory to be generated at every time step until it is successful
                if qbest_cost < 1e6:
