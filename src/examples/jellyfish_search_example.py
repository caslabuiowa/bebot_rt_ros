...
if __name__ == '__main__':
    ...
    assert np.allclose(c.cpts[:, 0], x), 'Initial position incorrect.'
    assert np.allclose(c.diff().cpts[:, 0], xdot), 'Initial velocity incorrect.'
    assert np.allclose(c.diff().diff().cpts[:, 0], xddot), 'Initial acceleration incorrect.'

    cost_fn = SumOfDistance(goal) + 50*DistanceToGoal(goal)
    constraints = [
